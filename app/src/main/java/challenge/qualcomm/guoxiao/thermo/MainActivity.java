...
public class MainActivity extends AppCompatActivity implements SensorEventListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(temperature != null) {
            if(cels) {
                temp.setText((int) temperature.getPower());
            } else {
                temp.setText(jniTranform((int)temperature.getPower()));
            }
        } else {
            temp.setText("-");
        }
    ...
    public void onSensorChanged(SensorEvent event) {
        if(cels) {
            temp.setText((int) event.values[0]);
        } else {
            temp.setText(jniTranform((int) event.values[0]));
        }
    }
